...
def extract_paragraph(is_word):
    ...
    con_list = []
    for name in txt_list:
        count = int(len(dict[name]) / word_len * 100 + 0.5)
        # 特殊处理
        if count ==0:
            count = 1
        #if name == '越女剑':
        ...
        for i in range(count):
            data_temp = dict[name][i * pos:i * pos + 20]
            con = {
                'number': number,
                'label': name,
                'data': data_temp
...
def extract_paragraph1(is_word):
    ...
    con_list = []
    for name in txt_list:
        count = int(len(dict[name]) / word_len * 900 + 0.5)
        # 特殊处理
        if count ==0:
            count = 1
        #if name == '越女剑':
        ...
        data_temp=[]
        for i in range(count):
            data_temp = data_temp + dict[name][i * pos:i * pos + 20]
        con = {
            'number': number,
            'label': name,
            'data': data_temp
